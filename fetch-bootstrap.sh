...
if [[ -z "${V_CHAIN_DATA_DIR-}" ]]; then
  if [[ "$OSTYPE" == "darwin"* ]]; then
    V_CHAIN_DATA_DIR="$HOME/Library/Application Support/Komodo/$V_CHAIN"
  else
    V_CHAIN_DATA_DIR="$HOME/.komodo/$V_CHAIN"
  fi
fi
...
OVERWRITE_V_DATA="${OVERWRITE_V_DATA:-}"
...
EOF
  ...
  if [ ${#vrsc_data[*]} -lt 1 ]; then
    ...
    echo "Found existing ${V_CHAIN} data:"
    echo "####################################################################################"
    for item in "${vrsc_data[@]}"; do
      echo "${item}"
    done
    echo "####################################################################################"
    if [ -n "$OVERWRITE_V_DATA" ]; then
